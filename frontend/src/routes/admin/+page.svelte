...
<div class="app-container min-h-screen" class:dark-mode={isDarkMode}>
          ...
          <table class="table w-full {isDarkMode ? 'text-white' : ''}">
            ...
            <tbody>
              {#each filteredBooks as book}
                <tr class="{isDarkMode ? 'hover:bg-gray-700' : ''}">
                  <td>{book.code}</td>
                  ...
                  <td>
                    <div class="flex gap-2">
                      <button class="btn btn-sm btn-ghost {isDarkMode ? 'hover:bg-gray-600' : ''}" on:click={() => openEditModal(book)}>
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                      </button>
                      <button class="btn btn-sm btn-ghost text-error {isDarkMode ? 'hover:bg-gray-600' : ''}" on:click={() => handleDeleteBook(book.id)}>
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 <div class="modal modal-open">
   <div class="modal-box {isDarkMode ? 'bg-gray-800 text-white' : ''}">
     <h3 class="font-bold text-lg">Aggiungi Nuovo Libro</h3>
     <div class="form-control gap-4 mt-4">
       <input type="text" placeholder="Codice" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.code} />
       <input type="text" placeholder="Titolo" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.title} />
       <input type="text" placeholder="Autore" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.author} />
       <input type="text" placeholder="CDD" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.cdd} />
       <input type="text" placeholder="Numero inventario" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.num_inventory} />
       <select class="select select-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.library}>
         <option value="">Seleziona biblioteca</option>
         {#each libraries as library}
           <option value={library.name}>{library.name}</option>
         {/each}
       </select>
        <select class="select select-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={newBook.category}>
        ...
        <label class="label cursor-pointer">
          <span class="label-text {isDarkMode ? 'text-white' : ''}">Disponibile</span>
          <input type="checkbox" class="checkbox" bind:checked={newBook.available} />
        </label>
      </div>
      <div class="modal-action">
        <button class="btn {isDarkMode ? 'bg-gray-700 hover:bg-gray-600 text-white' : ''}" on:click={() => showAddBookModal = false}>Annulla</button>
...
<div class="modal modal-open">
  <div class="modal-box {isDarkMode ? 'bg-gray-800 text-white' : ''}">
    ...
    <div class="form-control gap-4 mt-4">
      <input type="text" placeholder="Codice" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.code} />
      <input type="text" placeholder="Titolo" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.title} />
      <input type="text" placeholder="Autore" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.author} />
      <input type="text" placeholder="CDD" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.cdd} />
      <input type="text" placeholder="Numero inventario" class="input input-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.num_inventory} />
      <select class="select select-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.library}>
        <option value="">Seleziona biblioteca</option>
        {#each libraries as library}
          <option value={library.name}>{library.name}</option>
        {/each}
      </select>
       <select class="select select-bordered {isDarkMode ? 'bg-gray-700 text-white border-gray-600' : ''}" bind:value={editingBook.category}>
         <option value="">Seleziona categoria</option>
         {#each categories as category}
           <option value={category}>{category}</option>
         {/each}
      ...
      <div class="modal-action">
        <button class="btn {isDarkMode ? 'bg-gray-700 hover:bg-gray-600 text-white' : ''}" on:click={() => showEditBookModal = false}>Annulla</button>
